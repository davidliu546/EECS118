# graphicsDisplay.py
# ------------------
# Based on graphicsDisplay.py from the UC Berkeley Pacman AI Projects.
...
from graphicsUtils import _canvas
...
class PacmanGraphics:
    def __init__(self, zoom=1.0, frameTime=0.0, capture=False):
        ...
        #TODO [FAR] Allow custom pacman image
        self.pacmanImage = "catman.png"
        self.pacLeft = None
        self.pacRight = None
        self.pacmanSprite = None # [FAR] lazy initialization
        # self.pacmanSprite = tkinter.PhotoImage(file=self.pacmanImage)
        self.zoom = zoom
    ...
    def initialize(self, state, isBlue = False):
        ...
        #TODO [FAR] Use image for pacman
        # self.make_window(self.layout.width, self.layout.height)
    ...
    def drawAgentObjects(self, state):
        ...
        for index, agent in enumerate(state.agentStates):
            ...
            print("AGENT INDEX:", index, "TYPE:", agentState.isPacman)
            if agent.isPacman:
    ...
    def drawPacman(self, pacman, index):
        #TODO [FAR] Implement custom pacman image
        if self.pacmanImage != None:
            if self.pacmanSprite == None:
                self.pacLeft = tkinter.PhotoImage(file=self.pacmanImage)
                self.pacRight = self.pacLeft.copy()
                self.pacRight = self.pacRight.subsample( -1, 1 )
                self.pacmanSprite = self.pacLeft
                # self.pacmanSprite = tkinter.PhotoImage(file=self.pacmanImage)
            
            # print("DRAW PACMAN CALLED")
    
            position = self.getPosition(pacman)
            x, y = self.to_screen(position)
            r = PACMAN_SCALE * self.gridSize
    
            # use the global graphicsUtils.image wrapper
            # img_id = image(
            #     (x - r, y - r),
            #     file=self.pacmanImage
            # )
            # print("PAC POS:", position, "SCREEN:", x - r, y - r)
            img_id = graphicsUtils._canvas.create_image(
                # x - r, y - r,
                # anchor="nw",
                # image=self.pacmanSprite
                x, y,
                image=self.pacmanSprite,
                anchor="center"
            )
    ...
    def movePacman(self, position, direction, image):
        #TODO [FAR] Implement custom pacman image movement
        if self.pacmanImage != None:
            x, y = self.to_screen(position)
            r = PACMAN_SCALE * self.gridSize
            # move_to(image[0], x - r, y - r)
            # refresh()
            # graphicsUtils._canvas.coords(image[0], x - r, y - r)
            graphicsUtils._canvas.coords(image[0], x, y)
        else:
    ...
    def animatePacman(self, pacman, prevPacman, image):
        ...
        if self.frameTime > 0.01 or self.frameTime < 0:
            ...
            if direction == "West":
                graphicsUtils._canvas.itemconfigure(image[0], image=self.pacLeft)
            elif direction == "East":
                graphicsUtils._canvas.itemconfigure(image[0], image=self.pacRight)
            ...
            for i in range(1, int(frames) + 1):
                ...
                if self.pacmanImage is not None:
                    # move + bounce in the SAME frame loop (no extra pause)
                    x, y = self.to_screen(pos)
                    bounce = 3 * math.sin(2 * math.pi * t)  # 1 full bounce per move
                    graphicsUtils._canvas.coords(image[0], x, y + bounce)
                else:
                    self.movePacman(pos, direction, image)
    ...
    #     if self.pacmanImage != None:
    #         img_id = image[0]
    #         x, y = graphicsUtils._canvas.coords(img_id)
    #         bounce = 3                     # pixels
    #         for _ in range(3):
    #             graphicsUtils._canvas.coords(img_id, x, y - bounce)
    #             graphicsUtils.refresh()
    #             graphicsUtils.sleep(0.03)
    #             graphicsUtils._canvas.coords(img_id, x, y + bounce)
    #             graphicsUtils.refresh()
    #             graphicsUtils.sleep(0.03)
    ...
    def to_screen(self, point):
        ...
        #     y = (self.height - y - 1) * self.gridSize + self.gridSize / 2
        #     return (x, y)
        # else:
            ( x, y ) = point
            #y = self.height - y
